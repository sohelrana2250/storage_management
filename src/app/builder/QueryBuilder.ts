...
class QueryBuilder<T> {
    ...
    search(searchableFields: string[]) {
        ...
        if (searchTerm) {
            ...
            this.modelQuery = this.modelQuery.find({
                $or: stringFields.map(
                    (field) => ({ [field]: { $regex: searchTerm, $options: 'i' } }) as FilterQuery<T>
                ),
            });
        }
    ...
    filter()
    {
         ...
         {   
            queryObject={releaseDate:{$gte:this.query?.releaseDate as string,$lte:this.query?.releaseDate as string }}
         }
